...
public class ResetGenerateVotingCardsAndUpdateCommunalContestDeadlinesTest : BaseWriteableDbGrpcTest<ContestService.ContestServiceClient>
{
    public ResetGenerateVotingCardsAndUpdateCommunalContestDeadlinesTest(TestApplicationFactory factory)
        : base(factory)
    {
    }
    ...
    public override async Task InitializeAsync()
    {
        ...
        await ModifyDbEntities<Contest>(
            ...
            x => x.DeliveryToPostDeadline = MockedClock.GetDate());
    ...
    [Fact]
    public async Task ShouldUpdateContestDeadlines()
    {
        ...
        await AbraxasPrintJobManagerClient.ResetGenerateVotingCardsAndUpdateCommunalContestDeadlinesAsync(NewRequest());
        ...
        contest.AttachmentDeliveryDeadline.Should().Be(MockedClock.GetDate(105).Date.NextUtcDate(true));
        contest.DeliveryToPostDeadline.Should().Be(MockedClock.GetDate(110).Date.NextUtcDate(true));
    ...
    {
        await AbraxasPrintJobManagerClient.ResetGenerateVotingCardsAndUpdateCommunalContestDeadlinesAsync(NewRequest(d => d.PrintingCenterSignUpDeadlineDate = MockedClock.GetTimestampDate(0)));
    ...
    {
        await AssertStatus(
            async () => await AbraxasPrintJobManagerClient.ResetGenerateVotingCardsAndUpdateCommunalContestDeadlinesAsync(
    ...
    [Fact]
    public async Task ShouldThrowIfContestNotApproved()
    {
        await ModifyDbEntities<StepState>(
            x => x.DomainOfInfluenceId == DomainOfInfluenceMockData.ContestBundFutureApprovedBundGuid && x.Step == Step.ContestApproval,
            x => x.Approved = false);
    ...
    public async Task ShouldThrowIfNonCommunalContest()
    {
        await ModifyDbEntities<ContestDomainOfInfluence>(
            x => x.BasisDomainOfInfluenceId == DomainOfInfluenceMockData.BundGuid,
            x => x.Type = DomainOfInfluenceType.Ch);

        await AssertStatus(
            async () => await AbraxasPrintJobManagerClient.ResetGenerateVotingCardsAndUpdateCommunalContestDeadlinesAsync(
                NewRequest()),
            StatusCode.InvalidArgument,
            "Cannot set communal deadlines on non-communal contest");
    }
    ...
    {
        await service.ResetGenerateVotingCardsAndUpdateCommunalContestDeadlinesAsync(NewRequest());
    }
    ...
    private static ResetGenerateVotingCardsAndUpdateCommunalContestDeadlinesRequest NewRequest(Action<ResetGenerateVotingCardsAndUpdateCommunalContestDeadlinesRequest>? customizer = null)
    {
        var request = new ResetGenerateVotingCardsAndUpdateCommunalContestDeadlinesRequest
        {
            ...
            AttachmentDeliveryDeadlineDate = MockedClock.GetTimestampDate(105),
            DeliveryToPostDeadlineDate = MockedClock.GetTimestampDate(110),
