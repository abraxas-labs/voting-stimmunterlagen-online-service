...
public class RevertApprovePoliticalBusinessesStepTest : BaseWriteableStepTest
{
    public RevertApprovePoliticalBusinessesStepTest(TestApplicationFactory factory)
        : base(factory)
    {
    }
    ...
    public async Task ShouldWork()
    {
        ...
        printJobs.All(x => x.State == Data.Models.PrintJobState.Empty).Should().BeTrue();
    }
    ...
    public async Task ShouldNotUpdatePrintJobStateIfAllAttachmentsOnceDelivered()
    {
        ...
        var printJobs = await RunOnDb(db => db.PrintJobs
            ...
            .ToListAsync());
        printJobs.Should().NotBeEmpty();
        printJobs.All(x => x.State == Data.Models.PrintJobState.Empty).Should().BeFalse();
